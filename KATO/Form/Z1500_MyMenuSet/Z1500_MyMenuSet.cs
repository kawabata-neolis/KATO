                ...
                if (dtGetData.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtGetData.Rows)
                    {
                        if (int.Parse(dr.ItemArray[0].ToString()) >= 1000)
                        {
                            ...
                            if (cs.Length > 0)
                            {
                                ((LabelSet_Menu)cs[0]).CodeTxtText += dtGetData.Rows[intRowsCnt]["ＰＧ番号"];
                                ((LabelSet_Menu)cs[0]).ValueLabelText += dtGetData.Rows[intRowsCnt]["ＰＧ名"];
                            }


                            //// 現在のフォーム内から、textBox1 という名前のコントロールを探す
                            //Control cFindControl = FindControl(this, "labelSet_Menu1");

                            //cFindControl.Text = "1";

                            //LabelSet_Menu lblSetMenu = new LabelSet_Menu(cFindControl);
