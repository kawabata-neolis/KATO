...
dbo.f_getメーカー名(a.メーカーコード)  
 + ' ' + dbo.f_get中分類名(a.大分類コード,a.中分類コード) +  ' '  +  Rtrim(ISNULL(a.Ｃ１,'')) 
 + ' ' + Rtrim(ISNULL(a.Ｃ２,''))
 + ' ' + Rtrim(ISNULL(a.Ｃ３,''))
 + ' ' + Rtrim(ISNULL(a.Ｃ４,''))
 + ' ' + Rtrim(ISNULL(a.Ｃ５,''))
 + ' ' + Rtrim(ISNULL(a.Ｃ６,'')) AS 品名,
a.発注数量,
a.発注単価,
a.納期,
...
b.登録日時
 FROM 出庫ヘッダ a,出庫明細 b
 WHERE a.伝票番号 = b.伝票番号 AND a.削除 ='N' AND b.削除 ='N' AND b.受注番号 = '{0}'
) Z 
