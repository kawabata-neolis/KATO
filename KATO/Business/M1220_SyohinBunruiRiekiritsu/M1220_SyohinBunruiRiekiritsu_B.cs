...
namespace KATO.Business.M1220_SyohinBunruiRiekiritsu
{
    ...
    class M1220_SyohinBunruiRiekiritsu_B
    {
        ...
        public DataTable getRiekiritsuList(List<string> lstSearchItem, List<string> lstSerachOrder)
        {
            ...
            // 並び順が得意先の場合
            if (lstSerachOrder[0].Equals("0"))
            {
                // A-Zの場合
                if (lstSerachOrder[1].Equals("0"))
                {
                    strSql += " ORDER BY 得意先コード,大分類コード,中分類コード,メーカーコード";
                }
                // Z-Aの場合
                else if (lstSerachOrder[1].Equals("1"))
                {
                    strSql += " ORDER BY 得意先コード DESC,大分類コード,中分類コード,メーカーコード";
                }
            }

            // 並び順が大分類の場合
            else if (lstSerachOrder[0].Equals("1"))
            {
                // A-Zの場合
                if (lstSerachOrder[1].Equals("0"))
                {
                    strSql += " ORDER BY 大分類コード,中分類コード,メーカーコード,得意先コード";
                }
                // Z-Aの場合
                else if (lstSerachOrder[1].Equals("1"))
                {
                    strSql += " ORDER BY 大分類コード DESC,中分類コード,メーカーコード,得意先コード";
                }
            }

            // 並び順が中分類の場合
            else if (lstSerachOrder[0].Equals("2"))
            {
                // A-Zの場合
                if (lstSerachOrder[1].Equals("0"))
                {
                    strSql += " ORDER BY 中分類コード,大分類コード,メーカーコード,得意先コード";
                }
                // Z-Aの場合
                else if (lstSerachOrder[1].Equals("1"))
                {
                    strSql += " ORDER BY 中分類コード DESC,大分類コード,メーカーコード,得意先コード";
                }
            }

            // 並び順がメーカーの場合
            else if (lstSerachOrder[0].Equals("3"))
            {
                // A-Zの場合
                if (lstSerachOrder[1].Equals("0"))
                {
                    strSql += " ORDER BY メーカーコード,大分類コード,中分類コード,得意先コード";
                }
                // Z-Aの場合
                else if (lstSerachOrder[1].Equals("1"))
                {
                    strSql += " ORDER BY メーカーコード DESC,大分類コード,中分類コード,得意先コード";
                }
            }

            // 並び順が利益率の場合
            else if (lstSerachOrder[0].Equals("4"))
            {
                // A-Zの場合
                if (lstSerachOrder[1].Equals("0"))
                {
                    strSql += " ORDER BY 利益率,得意先コード,大分類コード,中分類コード,メーカーコード";
                }
                // Z-Aの場合
                else if (lstSerachOrder[1].Equals("1"))
                {
                    strSql += " ORDER BY 利益率 DESC,得意先コード,大分類コード,中分類コード,メーカーコード";
                }
            }

            // 並び順が掛率の場合
            else if (lstSerachOrder[0].Equals("5"))
            {
                // A-Zの場合
                if (lstSerachOrder[1].Equals("0"))
                {
                    strSql += " ORDER BY 掛率,得意先コード,大分類コード,中分類コード,メーカーコード";
                }
                // Z-Aの場合
                else if (lstSerachOrder[1].Equals("1"))
                {
                    strSql += " ORDER BY 掛率 DESC,得意先コード,大分類コード,中分類コード,メーカーコード";
                }
            }
