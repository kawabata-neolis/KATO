...
namespace KATO.Business.H0210_MitsumoriInput
{
    class H0210_MitsumoriInput_B
    {
        ...
        public DataTable getMitsumoriDetail(string strNum)
        {
            ...
            strSQL += "       null as 印,";
            strSQL += "       a.品名型式,";
            strSQL += "       CONVERT(CHAR, a.数量, 126) as 数量,";
            strSQL += "       CONVERT(CHAR, b.定価, 126) as 定価,";
            strSQL += "       CONVERT(CHAR, a.売上単価, 126) as 見積単価,";
            strSQL += "       CONVERT(CHAR, 0.0, 126) as 掛率,";
            strSQL += "       CONVERT(CHAR, ROUND(a.売上単価 * a.数量 , 0), 126) as 金額,";
            strSQL += "       CONVERT(CHAR, a.仕入単価, 126) as 仕入単価,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利金額, 0), 126) as 粗利金額,";
            strSQL += "       CONVERT(CHAR, ROUND(a.率, 1), 126) as 率,";
            strSQL += "       a.備考,";
            strSQL += "       a.仕入先名,";
            strSQL += "       a.印刷フラグ,";
            strSQL += "       a.仕入先コード１,";
            strSQL += "       a.仕入先名１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入単価１, 0), 126) as 仕入単価１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入金額１, 0), 126) as 仕入金額１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利１, 0), 126) as 粗利１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利率１, 0), 126) as 粗利率１,";
            strSQL += "       a.仕入先コード２,";
            strSQL += "       a.仕入先名２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入単価２, 0), 126)  as 仕入単価２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入金額２, 0), 126)  as 仕入金額２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利２, 0), 126)  as 粗利２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利率２, 0), 126)  as 粗利率２,";
            strSQL += "       a.仕入先コード３,";
            strSQL += "       a.仕入先名３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入単価３, 0), 126)  as 仕入単価３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入金額３, 0), 126)  as 仕入金額３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利３, 0), 126)  as 粗利３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利率３, 1), 126)  as 粗利率３,";
            strSQL += "       a.仕入先コード４,";
            strSQL += "       a.仕入先名４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入単価４, 0), 126)  as 仕入単価４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入金額４, 0), 126)  as 仕入金額４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利４, 0), 126)  as 粗利４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利率４, 1), 126)  as 粗利率４,";
            strSQL += "       a.仕入先コード５,";
            strSQL += "       a.仕入先名５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入単価５, 0), 126)  as 仕入単価５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入金額５, 0), 126)  as 仕入金額５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利５, 0), 126)  as 粗利５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利率５, 1), 126)  as 粗利率５,";
            strSQL += "       a.仕入先コード６,";
            strSQL += "       a.仕入先名６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入単価６, 0), 126)  as 仕入単価６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.仕入金額６, 0), 126)  as 仕入金額６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利６, 0), 126)  as 粗利６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.粗利率６, 1), 126)  as 粗利率６,";
            strSQL += "       a.加工仕入先コード１,";
            strSQL += "       a.加工仕入先名１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入単価１, 0), 126)  as 加工仕入単価１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入金額１, 0), 126)  as 加工仕入金額１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利１, 0), 126)  as 加工粗利１,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利率１, 1), 126)  as 加工粗利率１,";
            strSQL += "       a.加工仕入先コード２,";
            strSQL += "       a.加工仕入先名２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入単価２, 0), 126)  as 加工仕入単価２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入金額２, 0), 126)  as 加工仕入金額２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利２, 0), 126)  as 加工粗利２,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利率２, 1), 126)  as 加工粗利率２,";
            strSQL += "       a.加工仕入先コード３,";
            strSQL += "       a.加工仕入先名３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入単価３, 0), 126)  as 加工仕入単価３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入金額３, 0), 126)  as 加工仕入金額３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利３, 0), 126)  as 加工粗利３,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利率３, 1), 126)  as 加工粗利率３,";
            strSQL += "       a.加工仕入先コード４,";
            strSQL += "       a.加工仕入先名４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入単価４, 0), 126)  as 加工仕入単価４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入金額４, 0), 126)  as 加工仕入金額４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利４, 0), 126)  as 加工粗利４,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利率４, 1), 126)  as 加工粗利率４,";
            strSQL += "       a.加工仕入先コード５,";
            strSQL += "       a.加工仕入先名５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入単価５, 0), 126)  as 加工仕入単価５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入金額５, 0), 126)  as 加工仕入金額５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利５, 0), 126)  as 加工粗利５,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利率５, 1), 126)  as 加工粗利率５,";
            strSQL += "       a.加工仕入先コード６,";
            strSQL += "       a.加工仕入先名６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入単価６, 0), 126)  as 加工仕入単価６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工仕入金額６, 0), 126)  as 加工仕入金額６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利６, 0), 126)  as 加工粗利６,";
            strSQL += "       CONVERT(CHAR, ROUND(a.加工粗利率６, 1), 126)  as 加工粗利率６,";
            strSQL += "       a.商品コード,";
            strSQL += "       a.大分類コード,";
            strSQL += "       a.中分類コード,";
            strSQL += "       a.メーカーコード,";
